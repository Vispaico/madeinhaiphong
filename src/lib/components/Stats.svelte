<script>
  ...
  const stats = [
    {
      label: "Words of strategic content published for clients",
      limit: 3,
      suffix: "M+",
      prefix: "$",
    },
    {
      label: "Companies repositioned in their marke",
      limit: 38,
      suffix: "+",
      prefix: "",
    },
    {
      label: "Average time from engagement to measurable results",
      limit: 60,
      suffix: " Days",
      prefix: "<",
    },
    {
      label: "Projects delivered across four continents",
      limit: 100,
      suffix: "+",
      prefix: "",
    },
      ...
      <div class="flex flex-col gap-2 md:items-center">
        ...
        >
          <span class="text-white/30 text-3xl md:text-5xl mr-1"
            >{stat.prefix}</span
          >
          <span class="counterData text-[#a08300]">0</span>
          <span class="text-white/60">{stat.suffix}</span>
        </h4>
        <p class="text-xs font-mono text-white/50 uppercase tracking-widest">
          {stat.label}
        </p>
      </div>
    {/each}
